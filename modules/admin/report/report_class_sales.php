...
class PDF extends FPDF{
	...
	function viewTable($connection){
		...
		$q1=$connection->prepare('SELECT * FROM tbl_class');
		$q1->execute();
		$r = $q1->fetchAll();
		...
		foreach ($r as $value){
			...
			$q=$connection->prepare('SELECT SUM(trni_amount) as totalamount FROM tbl_transitems,tbl_transaction WHERE tbl_transitems.trns_id = tbl_transaction.trns_id AND tbl_transaction.trns_date BETWEEN :start AND :end AND tbl_transitems.cls_id = :id');
			...
			while($data = $q->fetch(PDO::FETCH_OBJ)){
				$this->SetFont('Arial','', 11);
				$this->Cell(100,10,$value['cls_name'],1,0,'L');
